...
// Import SegQueryIter from seg_query_iter.rs
mod seg_query_iter;
use seg_query_iter::SegQueryIter;
...
fn run_benchmark_tasks(
    ...
    // Test limit 100 with SegQueryIter (early termination approach)
    add_bench_task_seg_query_iter(
        bench_group,
        bench_index,
        query_str,
        100,
        "limit100_seg_query_iter",
    );
...
fn add_bench_task_seg_query_iter(
    bench_group: &mut BenchGroup,
    bench_index: &BenchIndex,
    query_str: &str,
    limit: usize,
    ...
    let task_name = format!("{}_{}", query_str.replace(" ", "_"), collector_name);
    let query = bench_index.query_parser.parse_query(query_str).unwrap();
    let search_task = SegQueryIterSearchTask {
        searcher: bench_index.searcher.clone(),
        query,
        limit,
    };
...
impl<C: Collector> SearchTask<C> {
    ...
    pub fn run(&self) -> usize {
        ...
        {
            let mut docs = doc_set.into_iter().collect::<Vec<_>>();
            docs.sort();
            let mut count = 0;
...
// New search task using SegQueryIter with early termination
struct SegQueryIterSearchTask {
    searcher: Searcher,
    query: Box<dyn Query>,
    limit: usize,
}

impl SegQueryIterSearchTask {
    #[inline(never)]
    pub fn run(&self) -> usize {
        let mut collected = 0;

        // Iterate through all segments
        for seg_ord in 0..self.searcher.segment_readers().len() {
            // Create an iterator for this segment
            let mut seg_iter = SegQueryIter::new(seg_ord as u32, &self.searcher, &*self.query)
                .expect("Failed to create SegQueryIter");

            // Iterate through documents in this segment
            while let Some(_doc) = seg_iter.next_doc() {
                collected += 1;
